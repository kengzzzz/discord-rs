...
use crate::{
    context::Context,
    slices::{
        admin_config::AdminConfigSlice, ai_assistant::AiAssistantSlice,
        community_roles::CommunityRolesSlice, moderation::ModerationSlice,
        onboarding::OnboardingSlice, runtime::RuntimeSlice, warframe::WarframeSlice,
    },
};
...
impl FeatureRegistry {
    fn new() -> Self {
        Self {
            slices: vec![
                Box::new(AdminConfigSlice),
                Box::new(AiAssistantSlice),
                Box::new(CommunityRolesSlice),
                Box::new(ModerationSlice),
                Box::new(OnboardingSlice),
                Box::new(RuntimeSlice),
                Box::new(WarframeSlice),
            ],
        }
    }
    ...
    pub async fn handle_interaction(&self, ctx: Arc<Context>, interaction: Interaction) {
        ...
        match mem::take(&mut interaction.data) {
            ...
            _ => tracing::warn!("ignoring non-command interaction"),
        }
    }
    ...
    pub async fn dispatch_event(&self, ctx: Arc<Context>, event: Event) {
        match event {
            ...
            Event::ReactionAdd(boxed) => {
                ...
                for slice in &self.slices {
                    if slice
                        .handle_reaction_add(
                            ctx.clone(),
                            twilight_model::gateway::payload::incoming::ReactionAdd(event.clone()),
                        )
                        .await
                    {
                        break;
                ...
                for slice in &self.slices {
                    if slice
                        .handle_reaction_remove(
                            ctx.clone(),
                            twilight_model::gateway::payload::incoming::ReactionRemove(
                                event.clone(),
                            ),
                        )
                        .await
                ...
                for slice in &self.slices {
                    if slice
                        .handle_voice_state_update(
                            ctx.clone(),
                            twilight_model::gateway::payload::incoming::VoiceStateUpdate(
                                event.clone(),
                            ),
                        )
                        .await
            ...
            _ => tracing::debug!("event ignored by slice registry"),
        }
    }
}
