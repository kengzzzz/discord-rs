...
use crate::{
    ...
    defer_interaction, guild_command,
...
pub async fn handle_intro_modal(
    ...
    if let Err(e) = guild_command!(ctx.http, interaction, true, {
        ...
        let Some(details) = introduction::form::parse_modal(&data) else {
            let embed = introduction::embed::intro_error_embed()?;
            ctx.http
                .interaction(interaction.application_id)
                .update_response(&interaction.token)
                .embeds(Some(&[embed]))
        ...
        introduction::handler::handle_valid_intro(
            &ctx,
            user.id,
            guild_id,
            &intro_channel,
        ...
        .await?;
        let embed = introduction::embed::intro_success_embed(&guild_ref)?;
        ctx.http
            .interaction(interaction.application_id)
            .update_response(&interaction.token)
            .embeds(Some(&[embed]))
    ...
    .await
    {
        ...
        if let Ok(embed) = introduction::embed::intro_error_embed()
            && let Err(e2) = ctx
                .http
                .interaction(interaction.application_id)
                .update_response(&interaction.token)
